...
Why it is not replaced with another local S3 tool:
  Alternatives (RustFS, Garage, SeaweedFS) exist but are too immature
  for a structured learning series. They are not yet widely adopted
  in corporate environments and would add tool-learning overhead
  without career value.
```
...
loki:
  storage:
    ...
    s3:
      ...
      bucketnames: my-loki-demo-bucket
      # Authentication via IRSA — no access keys in config
      # Pod ServiceAccount annotated with IAM role ARN

  # IRSA annotation on Loki ServiceAccount:
  serviceAccount:
    annotations:
      eks.amazonaws.com/role-arn: arn:aws:iam::123456789012:role/loki-s3-role
```

**Mimir with AWS S3 (Demo 22):**
...
mimir:
  structuredConfig:
    blocks_storage:
      backend: s3
      s3:
        bucket_name: my-mimir-demo-bucket
        region: ap-southeast-2
        # IRSA authentication — no static keys
...
**Required IAM policy for Loki/Mimir S3 access:**
  ...
  "Statement": [
    {
      ...
      "Resource": [
        "arn:aws:s3:::my-loki-demo-bucket",
        "arn:aws:s3:::my-loki-demo-bucket/*"
      ]
    }
  ]
